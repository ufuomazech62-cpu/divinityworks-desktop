 ...
 */
export async function setUserContext(client: pg.PoolClient, userId: string): Promise<void> {
...
export class DBSessionRepo {
  userId: string;
  mutex: { run: (key: string, fn: () => Promise<any>) => Promise<any> };
  private locks: Map<string, Promise<any>> = new Map();

  constructor({ userId }: { userId: string }) {
    this.userId = userId;
    this.mutex = {
      run: (key: string, fn: () => Promise<any>) => {
  ...
  async create(event: any): Promise<void> {
    ...
    try {
      ...
      await client.query(
        ...
        [sessionId, this.userId, event.title || '', event.ts || new Date().toISOString(), events]
      );
    } finally {
      client.release();
    }
  ...
  async read(sessionId: string): Promise<any[]> {
    ...
    try {
      ...
      const res = await client.query(
        `SELECT events FROM sessions WHERE id = $1 AND user_id = $2`,
  ...
  async append(sessionId: string, events: any[]): Promise<void> {
    ...
    try {
      ...
      await client.query(
        ...
        [sessionId, this.userId, JSON.stringify(events), new Date().toISOString()]
      );
    } finally {
      client.release();
    }
  ...
  async withLock(sessionId: string, fn: () => Promise<any>): Promise<any> {
  ...
  async setTitle(sessionId: string, title: string): Promise<void> {
    ...
    try {
      await setUserContext(client, this.userId);
      await client.query(
        `UPDATE sessions SET title = $3, updated_at = $4 WHERE id = $1 AND user_id = $2`,
        [sessionId, this.userId, title, new Date().toISOString()]
      );
    } finally {
      client.release();
    }
   ...
   */
  async upsert(sessionId: string, events: any[], title?: string, createdAt?: string): Promise<void> {
    const pool = getDbPool();
    const client = await pool.connect();
    try {
      await setUserContext(client, this.userId);
      await client.query(
        `INSERT INTO sessions (id, user_id, title, created_at, updated_at, events)
         VALUES ($1, $2, $3, $4, $4, $5)
         ON CONFLICT (id) DO UPDATE SET events = $5, title = $3, updated_at = $4`,
        [sessionId, this.userId, title || '', createdAt || new Date().toISOString(), JSON.stringify(events)]
      );
    } finally {
      client.release();
    }
...
export class DBTurnRepo {
    ...
    try {
      ...
      await client.query(
        `INSERT INTO turns (id, user_id, session_id, created_at, events)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (id) DO NOTHING`,
        [event.turnId, this.userId, event.sessionId || '', event.ts || new Date().toISOString(), JSON.stringify([event])]
      );
    } finally {
      client.release();
    }
  ...
  async read(turnId: string): Promise<any[]> {
    const pool = getDbPool();
    const client = await pool.connect();
    try {
      await setUserContext(client, this.userId);
      const res = await client.query(
        `SELECT events FROM turns WHERE id = $1 AND user_id = $2`,
        [turnId, this.userId]
      );
      if (res.rows.length === 0) {
  ...
  async append(turnId: string, events: any[]): Promise<void> {
    if (events.length === 0) return;
    const pool = getDbPool();
    const client = await pool.connect();
    try {
      ...
      await client.query(
        `UPDATE turns
         SET events = events || $3::jsonb
         WHERE id = $1 AND user_id = $2`,
        [turnId, this.userId, JSON.stringify(events)]
      );
    } finally {
      client.release();
    }
  ...
  async withLock(turnId: string, fn: () => Promise<any>): Promise<any> {
    return this.mutex.run(turnId, fn);
  }

  async upsert(turnId: string, events: any[], sessionId?: string): Promise<void> {
    const pool = getDbPool();
    const client = await pool.connect();
    try {
      await setUserContext(client, this.userId);
      await client.query(
        ...
        [turnId, this.userId, sessionId || '', events[0]?.ts || new Date().toISOString(), JSON.stringify(events)]
      );
    } finally {
      client.release();
    }
...
export class DBRunsRepo {
  ...
  async create(opts: any): Promise<string> {
    const pool = getDbPool();
    const client = await pool.connect();
    try {
      await setUserContext(client, this.userId);
      ...
      await client.query(
        `INSERT INTO runs (id, user_id, work_dir, events)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (id) DO NOTHING`,
        [runId, this.userId, opts.workDir || '', JSON.stringify(opts.events || [])]
      );
      return runId;
    } finally {
      client.release();
  ...
  async fetch(runId: string): Promise<any> {
    const pool = getDbPool();
    const client = await pool.connect();
    try {
      await setUserContext(client, this.userId);
      const res = await client.query(
        `SELECT events, work_dir FROM runs WHERE id = $1 AND user_id = $2`,
        [runId, this.userId]
      );
  ...
  async list(cursor?: string): Promise<any[]> {
    const pool = getDbPool();
    const client = await pool.connect();
    try {
      await setUserContext(client, this.userId);
  ...
  async listByWorkDir(workDir: string): Promise<any[]> {
    const pool = getDbPool();
    const client = await pool.connect();
    try {
      await setUserContext(client, this.userId);
      const res = await client.query(
        `SELECT id, work_dir, created_at FROM runs WHERE user_id = $1 AND work_dir = $2 ORDER BY created_at DESC`,
        [this.userId, workDir]
      );
      return res.rows;
    } finally {
  ...
  async appendEvents(runId: string, events: any[]): Promise<void> {
    if (events.length === 0) return;
    const pool = getDbPool();
    const client = await pool.connect();
    try {
      await setUserContext(client, this.userId);
      await client.query(
        `UPDATE runs SET events = events || $3::jsonb WHERE id = $1 AND user_id = $2`,
        [runId, this.userId, JSON.stringify(events)]
      );
    } finally {
      client.release();
    }
...
export class BrowserSessionRepo {
  ...
  async saveSession(domain: string, cookies: any[], opts?: { lastUrl?: string; lastTitle?: string; faviconUrl?: string }): Promise<void> {
    const pool = getDbPool();
    const client = await pool.connect();
    try {
      await setUserContext(client, this.userId);
      await client.query(
        `INSERT INTO browser_sessions (user_id, domain, cookies, last_url, last_title, favicon_url, updated_at)
         VALUES ($1, $2, $3::jsonb, $4, $5, $6, now())
         ON CONFLICT (user_id, domain)
         DO UPDATE SET cookies = $3::jsonb, last_url = $4, last_title = $5, favicon_url = $6, updated_at = now()`,
        [this.userId, domain, JSON.stringify(cookies), opts?.lastUrl || null, opts?.lastTitle || null, opts?.faviconUrl || null]
      );
    } finally {
      client.release();
    }
  ...
  async getSessions(): Promise<any[]> {
    ...
    try {
      await setUserContext(client, this.userId);
      const res = await client.query(
        `SELECT domain, cookies, last_url, last_title, favicon_url, updated_at
         FROM browser_sessions WHERE user_id = $1 ORDER BY updated_at DESC`,
        [this.userId]
      );
  ...
  async getSessionCookies(domain: string): Promise<any[]> {
    const pool = getDbPool();
    const client = await pool.connect();
    try {
      await setUserContext(client, this.userId);
      const res = await client.query(
        `SELECT cookies FROM browser_sessions WHERE user_id = $1 AND domain = $2`,
        [this.userId, domain]
      );
  ...
  async deleteSession(domain: string): Promise<void> {
    ...
    try {
      await setUserContext(client, this.userId);
      await client.query(
        `DELETE FROM browser_sessions WHERE user_id = $1 AND domain = $2`,
        [this.userId, domain]
      );
    } finally {
      client.release();
