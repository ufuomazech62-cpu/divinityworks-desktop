...
export class RedisTurnEventHub {
  ...
  subscribeAll(handler: (event: any) => void): void {
    const sub = getRedisSubscriber();
...
export class RedisSessionCache {
  ...
  async set(sessionId: string, entry: any): Promise<void> {
  ...
  async list(): Promise<any[]> {
    ...
    return Object.values(vals).map((v: string) => JSON.parse(v));
  }
...
export async function closeRedis(): Promise<void> {
  ...
  if (publisher) promises.push(publisher.quit().then(() => {}));
  if (subscriber) promises.push(subscriber.quit().then(() => {}));
  if (cacheClient) promises.push(cacheClient.quit().then(() => {}));
