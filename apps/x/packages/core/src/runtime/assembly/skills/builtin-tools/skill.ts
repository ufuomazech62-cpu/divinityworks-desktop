...
Load this skill when creating or modifying agents that need access to Divinity's builtin tools (shell execution, file operations, etc.).
...
The Divinity copilot has access to special builtin tools that regular agents don't typically use. These tools help the copilot assist users with file management, app workflows, and MCP integration:
...
- \`loadSkill\` - Load a Divinity skill definition into context
